...
std::map<int, std::string> ServerConfig::findErrorPage(
...
void ServerConfig::findLocation(std::string &line, ServerConfig &config,
                                std::ifstream &file,
                                std::string currentLocation)
{
  ...
  while (true) //(if line or next_line has '{')
  {
    ...
    Directory listing is a feature that allows the server to display the
    contents of a directory if no specific file is requested. When enabled,
    accessing a directory URL without a specified file will list all files and
